...
void cloudbox(int x1,int y1,int x2,int y2,int g1,int g2,int g3,int g4){
  ...
  // color edges
  gfx_color(g12,g12,255);
  gfx_line(x12,y1,x12,y1);
  gfx_color(g13,g13,255);
  gfx_line(x1,y12,x1,y12);
  gfx_color(g24,g24,255);
  gfx_line(x2,y12,x2,y12);
  gfx_color(g34,g34,255);
  gfx_line(x12,y2,x12,y2);

  // color center
  g = (g1+g2+g3+g4)/4 + pow(-1,rand()%2)*(rand()%(abs(x2-x1)/2+1)); // average of corners, plus/minus random fluctuation based on distance
  if(g>255) g=255;
  if(g<0) g=0;
  gfx_color(g,g,255);
